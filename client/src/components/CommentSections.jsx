...
export default function CommentSections({ postId }) {
  ...
  const handleSubmit = async (e)=>{
         ...
         try {
          ...
          if(res.ok)
          {
              ...
              setComments([data, ...comments]) // new comment added in comments array
          }
         } catch (error) {
  ...
  const handleEdit = (updatedComment, editedContent) => {
    setComments(
      comments.map((c) =>
        c._id === updatedComment._id ? { ...c, content: editedContent } : c
      )
    );
  };
    ...
    setShowModal(false);
     try {
        ...
        if (res.ok) {
            const data = await res.json();
            setComments(
              comments.filter((commenti) => commenti._id !== commentId)
            ); 
        }
     } catch (error) {
      console.log(error.message)
  ...
  return (
    <div className="max-w-2xl mx-auto w-full p-3">
      ...
      {currentUser && (
            <form onSubmit={handleSubmit} className="border border-teal-500 rounded-md p-5">
                <TextInput
                ...
                <Modal.Body>
                    <div className="text-center">
                        ...
                        <div className="flex justify-center gap-4">
                            {/* <Button color="failure" onClick={onClick={()=> handleDelete(commentToDelete)}}>Yes, I,m sure </Button> */}
